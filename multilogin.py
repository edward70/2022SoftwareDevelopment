...
logins = {"edward":"password", "user":"pass", "abc":"123"}
...
def authenticate():
    ...
    for user in logins.keys():
        if user == usern and logins[user] == passwd:
            outcome.setText("login successful")
            break
        else:
            outcome.setText("login failed")
